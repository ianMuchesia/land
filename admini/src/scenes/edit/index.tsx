...
dispatch(setFormLoader())
  try {
    await axios.patch(`${baseURL}/properties/${propertyID}`,  {
      title,
      area, price, description, location
  ...
  return (
    ...
    <div className="flex flex-col gap-9">
      ...
      <form onSubmit={handleSubmit} className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark" >
        ...
        <EditImage mainImage={createForm?.mainImage} images={createForm?.images} property={propertyID}/>
        </div>
        </div>
        </div>
  )
